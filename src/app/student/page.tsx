...
export default async function StudentDashboard() {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl lg:text-3xl font-bold text-slate-800">Welcome, {student?.name}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {[
          ...
          <div key={item.label} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 mb-5">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Link href="/student/view-attendance" className="flex items-center gap-3 px-5 py-3 bg-slate-100 text-slate-700 rounded-xl font-medium text-sm hover:bg-slate-200 transition-colors">
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
          <h2 className="font-semibold text-slate-700 mb-4 text-sm">My Profile</h2>
          <div className="space-y-3">
            {[
              ...
              <div key={item.label} className="flex justify-between text-sm">
                ...
                <span className="font-medium text-slate-700 text-right ml-4 truncate">{item.value || 'â€”'}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
            ...
            <Link href="/student/view-attendance" className="text-xs text-blue-600 hover:underline">View all â†’</Link>
          </div>
            ...
            {recentRecs?.length ? recentRecs.map((r) => {
              ...
              return (
                <div key={rec.id} className="flex items-center justify-between px-5 py-3">
                  <div>
                    <p className="text-sm font-medium text-slate-800">{rec.subjects?.name}</p>
                    <p className="text-xs text-slate-400">{rec.timetable?.day} Â· {rec.timetable?.start_time}</p>
                  </div>
                  <div className="text-right flex-shrink-0 ml-3">
                    <span className="inline-block bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs">Present</span>
                    <p className="text-xs text-slate-400 mt-0.5">{rec.date}</p>
                  </div>
                </div>
              );
            }) : (
              <p className="text-center py-8 text-slate-400 text-sm">No attendance records yet</p>
            )}
          </div>
        </div>
      </div>
      ...
      {/* 75% Rule reminder */}
      <div className="mt-5 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 text-sm text-amber-800">
        <span className="font-semibold">ðŸ“Œ UoS Attendance Rule:</span> Minimum 75% attendance is required per subject to be eligible for final exams.{' '}
        <Link href="/student/view-attendance" className="underline font-medium">Check your attendance â†’</Link>
      </div>
    </div>
  );
}
